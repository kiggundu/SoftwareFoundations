...
Example test_minus_6_5: (minus 6 5) = 1.
Proof. simpl. reflexivity. Qed.
End NatPlayground2.
...
Example plus_O_n' : forall n : nat, 0 + n = n.
Proof.
...
(* FILL IN HERE *)
Example test_bin_incr0 : (incr Z) = (B Z).
...
Proof. reflexivity. Qed.
Example test_bin_incr6 : (incr (A (B (B Z)))) = (B (B (B Z))).
Proof. reflexivity. Qed.
Example test_bin_incr7 : (incr (B (B (B Z)))) = (A (A (A (B Z)))).
Proof. reflexivity. Qed.

Example test_bin_to_nat1 : (bin_to_nat Z) = 0.
Proof. reflexivity. Qed.
Example test_bin_to_nat11 : (bin_to_nat (A Z)) = 0.
Proof. reflexivity. Qed.
...
Proof. reflexivity. Qed.
Example test_bin_to_nat6 : (bin_to_nat (B (A (B Z)))) = 5.
Proof. reflexivity. Qed.
Example test_bin_to_nat7 : (bin_to_nat (A (B (B Z)))) = 6.
Proof. reflexivity. Qed.
Example test_bin_to_nat8 : (bin_to_nat (B (B (B Z)))) = 7.
Proof. reflexivity. Qed.

(*
Example incr_never_0 : forall(b : bin), bin_to_nat(incr(b)) <> 0.
Proof. intros. 
  - destruct incr. reflexivity. Qed.

*)

Lemma nat_plus_commute: forall(n m : nat), (n + 0) = (0 + n).
Proof.
  intros [].
    - reflexivity.
    - rewrite -> plus_O_n. rewrite <- plus_n_O. intros. reflexivity.
Qed.


Lemma bin2nat_equiv: forall (b1 b2: bin), ((b1 = b2) -> bin_to_nat(b1) = bin_to_nat(b2)).
Proof.
  intros. rewrite <- H. reflexivity.
Qed.

Lemma incr_equiv: forall (b1 b2: bin), ((b1 = b2) -> incr(b1) = incr(b2)).
Proof.
  intros. rewrite <- H. reflexivity.
Qed.
...
(*
To prove forall (b : bin), ((S (bin_to_nat(b))) = (bin_to_nat(incr b))) either:

1- To prove: forall (b : bin),  ((S (bin_to_nat(b))) = (bin_to_nat(incr b)))
I have to be able to spit the bin type such that [incr] consumes a bin but returs a type restricted to non-zero bin members. With this tighter (more accurate) type I can properly destruct incr  (Haven't learned that yet)

2- To prove forall (b : bin), (bin_to_nat (incr b) > 0) -> ((S (bin_to_nat(b))) = (bin_to_nat(incr b))).
I need to be able to use [gt_Sn_O] in [Coq.Arith.Arith] or [nat_compare_gt] to prove
Theorem incr_convert_commutative : forall (b : bin), (bin_to_nat (incr b) > 0) -> ((S (bin_to_nat(b))) = (bin_to_nat(incr b))).
SOmething I haven't learned yet
*)

Definition P_bin_gt_Z (b : bin): bool :=
  match b with
  | Z => false
  | _ => true
  end.
...
Theorem incr_convert_commutative : forall (b : bin),  ((incr b) <> Z) -> (((S (bin_to_nat(b))) = (bin_to_nat(incr b)))).
Proof. 
  intro . destruct incr. 
    - intros. simpl. contradiction.
    - intros. destruct (A b0). 
intro. unfold bin_to_nat. simpl. apply H. auto.  
  - intros. absurd.
induction H. simpl. unfold incr. rewrite -> H. destruct b eqn:EH.
*)

(*
Theorem incr_convert_commutative : forall (b : bin),  exists m:nat, (bin_to_nat (incr b) = S m) -> ((S (bin_to_nat(b))) = (bin_to_nat(incr b))).
Proof.
intros . simpl. intros. destruct b.
- intros. 
{ destruct (bin_to_nat contra).
  - reflexivity.
  - destruct (S (S n) = 1).

rewrite <- gt_Sn_O. simpl. reflexivity. 
destruct (bin_to_nat b). nat_
- 
discriminate.
destruct (bin_to_nat(incr b)) eqn:Eb.
  - { simpl. reflexivity. }
  - { destruct b0 eqn:Eb0. 
      - simpl. reflexivity.
      - simpl. 
  ...
  - { intros. destruct (incr(B b0) ) eqn:Eb0. 
