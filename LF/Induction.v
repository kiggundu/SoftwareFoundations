(*
Add LoadPath "/Users/abrahamkiggundu/workspace/coq-stuff/software foubations/lf".
*)
...
Lemma plus_swap_Sm_n: forall (m n :nat), ((S m) + n) = (m + (S n)).
Proof.
...
(* Destruct breaks down a theorem by case analysis such that a subgoal is created for each constructor of the type of the variable to be case analysed. Induction however creates first a goal to prove the theorem for the first constructor. If proved an inductive hypothesis stating that the theorem is true for the next constructor is introduced at the goal to prove the case for the next constructor. If this hypothesis doe not lead to a contradiction then we conclude that it is true for the next constructor  *)
*)
...
Proof.
  intros.
  assert ((n + (m + p)) = (n + m + p)) as HEx.
  - rewrite plus_assoc. reflexivity.
  - rewrite -> HEx. rewrite plus_assoc. 
    { assert ((m + n) = (n + m)) as HPlusCom.
      - rewrite plus_comm. reflexivity. 
      - rewrite <- HPlusCom. reflexivity. 
    }
Qed.
  (* FILL IN HERE *) 
...
Lemma mult_O_n: forall (n : nat), ((0 * n) = 0).
Proof.
...
Lemma plus_n_1: forall (n: nat), ((1 + n) = S n).
Proof.
  intros. 
  induction n as [|n' IHn'].
    - simpl. reflexivity.
...
Theorem mult_factor : forall (m n : nat), ((n + m * n) = n * (1 + m)).
Proof.
  intros.
  induction m as [| m' IHm'].
    - simpl. rewrite mult_n_1. rewrite plus_n_O. reflexivity.
    - simpl. rewrite IHm'. rewrite plus_n_1. rewrite <- plus_comm. rewrite mult_n_Sm. reflexivity.
Qed.
...
Proof.
  intros [].
  - intros. rewrite <- mult_n_O. rewrite mult_O_n. reflexivity.
  - intros. simpl. rewrite mult_factor. rewrite plus_n_1. reflexivity. 
Qed.
  (* FILL IN HERE *)
(** [] *)
...
Proof.
  ...
  - rewrite PeanoNat.Nat.mul_add_distr_r. reflexivity.
Qed.

Theorem mult_assoc : forall n m p : nat,
  n * (m * p) = (n * m) * p.
Proof.
  intros. rewrite PeanoNat.Nat.mul_assoc. reflexivity.
Qed.
